...
$("#modalForm").submit(function(e) {
    ...
    $.ajax({
           type: "POST",
           url: '/eng/send.php',
           data: {phone: $('#examplePhoneNo').val()}, // serializes the form's elements.
           success: function(data)
           {
               ...
               if(data == 'ok')
               {
                ...
                $('#myModal .modal-content .container .row').html(''+
                    '<div class="result">'    
                    +'<img src="/eng/img/checked.png">'
                    +'<p class="h4">Успешно отправлено!</p>'
               ...
               } else
               {
                ...
                $('#myModal .modal-content .container .row').html(''+
                    '<div class="result">'  
                    +'<img src="/eng/img/error.png">'
                    +'<p class="h4">Ошибка отправки</p>'
                    +'<p>Мы уже чиним, а вы пока можете набрать менеджеру</p>'
                    +'<p>+375 29 618 80 90 (velcom)</p>'
                    +'</div>'
                    );
               }
           }
...
$("#mc-embedded-subscribe-form").submit(function(e){
    ...
    $.ajax({
        type:"POST",
        url:'/eng/send.php',
        data:{phone:$('#mc-email').val()},
        success:function(data)
        {
            if(data == 'ok')
           {
            console.log('отправилось');
            $('.mailchimp-success').text('Успешно отправлено!');
            $('#mc-email').val('');
            yaCounter50071654.reachGoal('form');
           } else
           {
            console.log('ошибка отправки');
            $('.mailchimp-error').text('Ошибка отправки, попробуйте позже');
           }
        }
